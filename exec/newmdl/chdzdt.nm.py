...
# =============================================
#          Classes for Words Classification
# =============================================

class CNNClassifier(nn.Module):
    def __init__(self, input_size, nb_classes, nb, a):
        super(CNNClassifier, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=nb, kernel_size=nb_classes)
        self.act1 = nn.ReLU()
        self.pool = nn.MaxPool1d(kernel_size=nb_classes)
        self.flatten = nn.Flatten()
        self.fc = nn.Linear(input_size, nb_classes)
...
class WordsCNNClassification:
    def __init__(self, tokenizer, word_encoder, max_words: int = 100):
    ...
    def encode_sentences(self, sentences: List[str]):
        ...
        tokens = [self.word_encoder.encode(token) for token in tokens]
        return tokens
    ...
    def train(self):
        ...
        # Example usage:
        # self.model = CNNClassifier(input_size=input_size, nb_classes=self.nb_classes, nb=nb, a=a)
        train_dataset = prepare_words_dataset(self.tokenizer, **self.config)
        trainer = train_model(self.encoder, self.tokenizer, train_dataset, None, **self.config)
        save_model(trainer, self.tokenizer, **self.config)
...
def main(args):
    ...
    tokenizer, encoder = load_model(**config)




parser = argparse.ArgumentParser(description="train a model using chDzDt encodings")
...
if __name__ == "__main__":
    ...
    argv = [
        "~/Data/DZDT/models/chdzdt_nm.json",
    ]
