...
def create_char_tokenizer(**kwargs) -> CharTokenizer:
    print("Creating character tokenizer")
    max_char_position = kwargs.get("max_char_position", MAX_CHAR_POSITION)
    char_tokenizer = CharTokenizer(max_position=max_char_position)

    charsets = kwargs.get("charsets", [])
    for charset in charsets:
        char_tokenizer.add_charset(charset[0], charset[1])

    chars = kwargs.get("chars", [])
    char_tokenizer.add_chars(chars)
...
if __name__ == "__main__":
    ...
    argv = [
        "new",
        # "resume",
        "~/Data/DZDT/models/chdzdt_train.json",
